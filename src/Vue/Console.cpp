...
void Console::menuAgency()
{
    ...
    cout << "+      - Pour chercher un capteur similaire                           4           +" << endl;
    cout << "+      - Pour revenir a  l'acceuil                                    5           +" << endl;
    cout << "+                                                                                 +" << endl;
    cout << "+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+" << endl;
    char choice[100];
    fscanf(stdin, "%99s", choice);
...
void Console::impactAirCleaner()
{
    ...
    fscanf(stdin, "%99s", choice);
    if (formatIR(choice))
    { //Si le choix de l'utilisateur est entré dans le bon format
    ...
    { //Si le format n'est pas bon, retour au menu impactCleaner
        badFormat("airCleaner1");
    }
}

void Console::similarSensor()
{
    //========================================Pour consulter l'impact d'un AirCleaner========================================
    cout << "+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+" << endl;
    cout << "+                                     AirWatcher                                  +" << endl;
    cout << "+                                                                                 +" << endl;
    cout << "+                              Entrez l'id du Sensor voulu                        +" << endl;
    cout << "+                             \"n°duCleaner\"                           +" << endl;
    cout << "+                                                                                 +" << endl;
    cout << "+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+" << endl;
    char choice[100];
    fscanf(stdin, "%99s", choice);
    ...
    try
    {
        ...
        start = clock();
        // Récupération du cleaner
        Sensor sens = airWatch.data.sensors.at(id);
        end = clock();
        temps = (float)(end - start);
        // Calcul de son impact
        list<pair<float, Sensor>> *similar = airWatch.findSimilarSensor(sens, 10, parseTime("2019-01-01 12:00:00"));
        //Affichage du résultat
        cout << "+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+" << endl;
        cout << "+                                     AirWatcher                                  +" << endl;
        cout << "+                                                                                 +" << endl;
        cout << "+                         Capteurs similaires à " << id << "                     +" << endl;
        cout << "+                          SensorId:                                                      +" << endl;
        int n = 0;
        for (auto it : *similar)
        {
    ...
    {
        cout << "+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+" << endl;
        cout << "|                           Aucun Sensor trouvé avec cet ID :                    |" << endl;
        cout << "|                                       " << id << "                                  |" << endl;
        cout << "+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+" << endl;
    }
    //Retour au menu d'accueil
...
void Console::selectZone()
{
    ...
    { //Si le format n'est pas bon, retour au menu statistics
        badFormat("stats");
    }
}
